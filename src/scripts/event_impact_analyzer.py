...
def fetch_news(query: str, lang: str = "en", limit: int = 5, retries: int = 2) -> list[dict]:
    ...
    return get_container().news().fetch_headlines(query, lang=lang, limit=limit)
...
def format_causal_summary(result: dict) -> str:
    ...
    lines.append(
        f"**総合市場方向: {dir_map.get(result['market_direction'], result['market_direction'])}**"
    )
    lines.append("")
    ...
    if result["triggered_rules"]:
        ...
        for _key, info in result["triggered_rules"].items():
    ...
    if result["sector_impacts"]:
        ...
        pos_sectors = {
            k: v for k, v in result["sector_impacts"].items() if v["direction"] == "positive"
        }
        neg_sectors = {
            k: v for k, v in result["sector_impacts"].items() if v["direction"] == "negative"
        }
