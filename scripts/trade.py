...
def load_or_create_broker(config: dict) -> BrokerSimulator:
    ...
    if PORTFOLIO_FILE.exists():
        ...
        print(f"⚠️  ポートフォリオファイルなし - 初期状態で開始")
...
def format_result_for_chat(result: dict) -> str:
    ...
    lines = [
        ...
        f"- **エントリー価格**: ${result['entry_price']:,.2f}" if result['entry_price'] > 0 else "- **エントリー価格**: (成行)",
        f"- **約定価格**: ${result['fill_price']:,.2f}" if result['fill_price'] else "- **約定価格**: (未約定)",
...
def cmd_check_positions(config: dict, risk_limits: dict) -> int:
    ...
    if summary["positions"]:
        ...
        for pos in summary["positions"]:
            ...
            print(f"- {pos['ticker']}: {pos['quantity']}株 @ ¥{entry} "
                f"(損益: {pnl} / {pnl_pct}%)")
    else:
        print("**保有ポジション:** なし")
...
def cmd_check_and_close_positions(config: dict, risk_limits: dict) -> int:
    ...
    if results:
        ...
        for result in results:
            ...
            print(f"{status_emoji} {result['ticker']}: {result['reason']} "
