...
未満の買い直しは却下される。最低保有期間内の AI 売り（trim/exit）は**当日合算で建玉の
50% まで**（trim 連射による実質全量売却の迂回を防ぐ）。機械ストップは常に無条件。
また submit_signals スキーマが、fail_conditions に短期テクニカル矛盾を自書きした
高確信度（0.6以上）エントリーを自己矛盾として却下する。
...
| 最低保有期間 | 2営業日 | AI 判断の swap/exit 売りを禁止、trim は当日合算50%まで（機械ストップは対象外） |
