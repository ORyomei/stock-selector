...
> **注意（歴史文書）**: 本書は売買エンジン初期設計時（Copilot Chat `/trade` 時代）の
> スナップショットであり、ファイルパス・コマンド例は現状と異なる。
> `src/scripts/trade.py` は現在 `uv run stock-selector trade` (typer CLI)、
> シミュレーターは `src/infra/brokers/` に移動済み。設計思想（ブローカー抽象化・
> 非LLM執行・リスク管理の層分離）は現在も有効。最新の構成は README とコードを参照。
